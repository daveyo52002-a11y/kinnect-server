...
// ── GET /api/rentals ─────────────────────────────────────────────
// Returns the current user's active rentals
...
// ── POST /api/rentals ────────────────────────────────────────────
// Body: { movie_id }
// Decrements stock_count atomically, creates rental record
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const { rows: movieRows } = await client.query(
      'SELECT movie_id, title, stock_count FROM movies WHERE movie_id = $1 FOR UPDATE',
      [movie_id]
    );
    const movie = movieRows[0];
    if (!movie)             { await client.query('ROLLBACK'); return res.status(404).json({ error: 'Movie not found' }); }
    if (movie.stock_count < 1) { await client.query('ROLLBACK'); return res.status(409).json({ error: 'No copies available' }); }

    // Check user doesn't already have an active rental for this movie
